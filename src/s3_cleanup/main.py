...
def handle_sort(args):
    ...
    print(
        f"Sorted file saved to '{args.output}' (sorted by {args.by}, reverse={args.reverse})"
    )
...
def handle_delete(args):
    ...
    confirm = input(
        f"Are you sure you want to permanently delete these files from '{bucket}'? (yes/no): "
    )
...
def main():
    ...
    scan_parser.add_argument(
        ...
        "--endpoint",
        default=None,
        help="S3 Endpoint URL (e.g. https://oceanstor.ip:port)",
    )
    ...
    usage_parser = subparsers.add_parser(
        "usage", help="Get fast estimate of total bucket / prefix usage"
    )
    ...
    summary_parser = subparsers.add_parser(
        "summary", help="Print total size and count from list file"
    )
    summary_parser.add_argument(
        "-i", "--input", default="old_files_list.txt", help="Input list file path"
    )
    ...
    delete_parser = subparsers.add_parser(
        "delete", help="Perform batch deletion from list file"
    )
    ...
    delete_parser.add_argument(
        ...
        type=float,
        default=100.0,
        help="Amount of storage to clean up (in GB)",
    )
    delete_parser.add_argument(
        "--dry-run", action="store_true", help="Simulate deletion without executing it"
    )
